...
for (analysis.round in c("1st", "2nd")){
  ...
  for (sample in names(stage_lst)){
    #####------------------------------------------------------------------#####
    # PATHS CONFIGURATIONS FOR EACH SAMPLE-RUN
    #####------------------------------------------------------------------#####
    path2input <- file.path(path.to.main.input, "GEX", sprintf("GEX_single_%s", sample))
    
    path.to.sample.output <- file.path(path.to.main.output, sprintf("/%s_round", analysis.round))
    dir.create(path.to.sample.output, showWarnings = FALSE)
    
    path.to.saved.html <- file.path(path.to.sample.output, sprintf("%s_%s_round", sample, analysis.round), "downstream_analysis_report")
    ...
    if (analysis.round == "2nd"){
      ...
      filtered.barcodes.1 <- readRDS(file.path(path.to.filtered.barcodes.1, sprintf("%s_1st_round/s9_output/remove_barcodes/%s_1st_round_remove_barcodes.rds", sample, sample)))
    ...
    s.obj <- run_pipeline_GEX(path2src=path2src,
                              ...
                              path.to.logfile.dir=file.path(path.to.sample.output, sprintf("%s_%s_round", sample, analysis.round), "logs"),
                              ...
                              save.RDS=save.RDS,
                              path.to.output=file.path(path.to.sample.output, sprintf("%s_%s_round", sample, analysis.round)),
                              rerun=rerun,
                              DE.test="wilcox",
                              num.PCA=num.PCA,
