...
path.to.main.output <- file.path(outdir, sprintf("%s_remove_c9", PROJECT))
...
stage_lst[["m330_merged"]] <- c( m330_merged = "m330")
stage_lst[["m331_merged"]] <- c( m331_merged = "m331")
...
filtered.barcodes <- readRDS(file.path(outdir, PROJECT, "/1st_round/pct_mito_10_1/data_analysis/01_output/remove_barcodes_cluster_9.rds"))
